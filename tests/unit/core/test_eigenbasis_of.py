...
class TestEigenbasisOf(unittest.TestCase):
    def setUp(self):
        ...
        hval = numpy.array([[0.1, 1.0], [1.0, 0.0]])
        dval = numpy.array([[0.1, 3.0], [3.0, 0.0]])
            ...
            with eigenbasis_of(self.H):
                ...
                with eigenbasis_of(self.B):
                    ...
                    with eigenbasis_of(self.P):
                        if knocks[2][0]:
                            self.assertTrue(numpy.allclose(self.H.data, h_copy_tr3))
                        if knocks[2][1]:
                            self.assertTrue(numpy.allclose(self.B.data, b_copy_tr3))
                        if knocks[2][2]:
                            self.assertTrue(numpy.allclose(self.P.data, p_copy_tr3))
                        if knocks[2][3]:
                            self.assertTrue(numpy.allclose(self.D.data, d_copy_tr3))

                        with eigenbasis_of(self.D):
                            if knocks[3][0]:
                                self.assertTrue(numpy.allclose(self.H.data, h_copy_tr4))
                            if knocks[3][1]:
                                self.assertTrue(numpy.allclose(self.B.data, b_copy_tr4))
                            if knocks[3][2]:
                                self.assertTrue(numpy.allclose(self.P.data, p_copy_tr4))
                            if knocks[3][3]:
                                self.assertTrue(numpy.allclose(self.D.data, d_copy_tr4))
