...
class TestRateMatrix(unittest.TestCase):
    ...
    def test_rate_matrix_creation(self):
        ...
        numpy.testing.assert_allclose(tst, RM, rtol=1.0e-5, atol=1.0e-12)

        # testing that creation without arguments raises exception
        with self.assertRaises(Exception) as context:
            RM2 = RateMatrix()
        self.assertTrue(
            "One of the arguments has to be specified" in str(context.exception)
        )
        ...
        self.assertTrue("Expecting rectangular matrix" in str(context.exception))

        # testing for wrong dimension specification
        with self.assertRaises(Exception) as context:
            RM = RateMatrix(data=tst, dim=6)
        self.assertTrue("Inconsistent data dimensions" in str(context.exception))

        # testing for correct dimension specification
        RM = RateMatrix(data=tst, dim=5)
        numpy.testing.assert_allclose(tst, RM.data, rtol=1.0e-5, atol=1.0e-12)
    ...
    def test_rate_value_setting(self):
        ...
        self.assertTrue(
            "Diagonal (depopulation) rates cannot be set" in str(context.exception)
        )
