...
class TDRedfieldFoersterRelaxationTensor(
    RedfieldFoersterRelaxationTensor, TimeDependent
):
    ...
    def _reference_implementation(self) -> None:
        ...
        if calcRT:
            if self._has_cutoff_time:
                RT = TDRedfieldRelaxationTensor(ham, sbi, cutoff_time=self.cutoff_time)
            else:
