...
if _repeate_:
    Js = [5.0, 10.0, 20.0, 30.0, 40.0, 50.0, 75.0, 100.0, 150.0, 200.0, 250.0, 300.0]
    # Js = [50.0, 100.0, 150.]
    # Js_nonzero = Js[1:]
else:
    Js = [5]
...
""",
    ...
    with qr.eigenbasis_of(ham):
        ...
        if _show_plots_:
            ...
            plt.plot(
                timea.data,
                numpy.real(rhot.data[:, 1, 2]),
                "--b",
                label="Coherence 1-2, real part",
            )
            plt.plot(
                timea.data,
                numpy.imag(rhot.data[:, 1, 2]),
                "--g",
    ...
    if _show_plots_:
        with qr.eigenbasis_of(ham):
            plt.plot(
                timea.data, numpy.real(rhot.data[:, 0, 1]), "-m", label="Real part"
            )
            plt.plot(
                timea.data,
                numpy.imag(rhot.data[:, 0, 1]),
                "--m",
                label="Imaginary part",
            )
            plt.plot(
                timea.data, numpy.imag(rhot.data[:, 0, 1]), "-k", label="Absolute value"
            )
    ...
    with qr.eigenbasis_of(ham):
        ...
        if _show_plots_:
            plt.plot(timea.data, numpy.abs(ext_coh), "-b", label="Electronic coherence")
            plt.plot(timea.data, numpy.abs(opt_coh), "-m", label="Optical coherence")
            plt.legend()
    ...
    if _show_plots_:
        plt.plot(timea.data, numpy.abs(ext_cohS), "-b", label="Electronic coherence")
        plt.plot(timea.data, numpy.abs(opt_cohS), "-m", label="Optical coherence")
        plt.legend()
    ...
    if _fit_:
        if J != 0.0:
            ...
            if _show_plots_:
                # plot the fit
                with qr.eigenbasis_of(ham):
                    plt.plot(timea.data, pop_e, "-k", label="Population of exciton 2")
                    plt.plot(
                        timea.data,
                        popt_p_e[0] * numpy.exp(-popt_p_e[1] * timea.data)
                        + popt_p_e[2],
                        "--g",
                        label="Fit of exciton 2",
                    )
                    # plt.show()

            print("Fitted population decay time (exciton basis): ", 1.0 / popt_p_e[1])
            print("Fitted population decay time (site basis): ", 1.0 / popt_p_s[1])
        else:
        ...
        if J == 0.0:
            ...
            print("Uphill decay time from canonical detailed balance: ", 1.0 / balance)

            rate_predicted = 0.5 * balance + 0.5 * rat
            print(
                "Prediction for the decoherence time based on population transfer:",
                1.0 / rate_predicted,
            )
        ...
        if _show_plots_:
            ...
            with qr.eigenbasis_of(ham):
                plt.plot(
                    timea.data, numpy.abs(opt_coh), "-m", label="optical coherence"
                )
                plt.plot(
                    timea.data,
                    popt_o[0] * numpy.exp(-popt_o[1] * timea.data) + popt_o[2],
                    "--b",
                    label="optical coherence fit",
                )
                plt.plot(
                    timea.data, numpy.abs(ext_coh), "-r", label="electronic coherence"
                )
                plt.plot(
                    timea.data,
                    popt_e[0] * numpy.exp(-popt_e[1] * timea.data) + popt_e[2],
                    "--k",
                    label="electronic coherence fit",
                )
                plt.legend()
                plt.title("Mono-exponential fits of coherence and population decay")
                plt.show()
            if J != 0.0:
                plt.plot(timea.data, pop_s, "-r", label="population")
                plt.plot(
                    timea.data,
                    popt_p_s[0] * numpy.exp(-popt_p_s[1] * timea.data) + popt_p_s[2],
                    "--k",
                    label="population fit",
                )
                plt.legend()
                plt.title("Mono-exponential fits of site basis population decay")
                plt.show()

        print("Fitted optical coherence decay time (exciton basis): ", 1.0 / popt_o[1])
        print(
            "Fitted electronic coherence decay time (exciton basis): ", 1.0 / popt_e[1]
        )
