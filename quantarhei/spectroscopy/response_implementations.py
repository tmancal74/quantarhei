            ...
            for bb in band1:
                ...
                for ff in band2:
                    ...
                    np.exp(
                        ...
                        + (np.einsum("i,ij", MM[b,f,:], gg[:,"t3"]))[None,:]
                        + (np.einsum("i,ij", MM[f,a,:], gg[:,"t1+t2"]))[:,None]
                        + (np.einsum("i,ij", MM[f,a,:], gg[:,"t3"]))[None,:]
                        - (np.einsum("i,ij", MM[f,f,:], gg[:,"t3"]))[None,:]
                        + (np.einsum("i,ijk", MM[b,a,:], gg[:,"t1+t2+t3"]))[:,:]
                        - (np.einsum("i,ijk", MM[f,a,:], gg[:,"t1+t2+t3"]))[:,:]
                        + np.conj((np.einsum("i,i", MM[a,b,:], gg[:,"t2"])))
                        - np.conj((np.einsum("i,i", MM[f,b,:], gg[:,"t2"])))
                        - np.conj((np.einsum("i,ij", MM[b,b,:], gg[:,"t2+t3"]))[None,:])
                        + np.conj((np.einsum("i,ij", MM[f,b,:], gg[:,"t2+t3"]))[None,:])
                        -1j*(En[aa]-En[g]-rwa)*t1[:,None]
                        -1j*(En[aa]-En[b])*t2[None,None]
