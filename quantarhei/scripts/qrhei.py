...
def do_command_run(args: Any) -> None:
    ...
    if args.benchmark > 0:
        ...
        qr.printlog("Running benchmark no. ", args.benchmark, verbose=True, loglevel=1)
    ...
    if not flag_quiet:
        ...
        qr.printlog(
            "Running Quantarhei (python) script file: ",
            scr,
            verbose=True,
    ...
    if flag_parallel:
        ...
        try:
            ...
            qr.printlog(
                " --- Exited by SystemExit --- ", verbose=True, loglevel=qr.LOG_DETAIL
            )
            pass
    ...
    if retval == 0:
        if not flag_silent and (not flag_quiet):
            ...
            qr.printlog(
                "Finished sucessfully; exit code: ", retval, verbose=True, loglevel=1
            )
    else:
        qr.printlog(
            "Warning, exit code: ", retval, verbose=True, loglevel=qr.LOG_URGENT
        )
...
def _match_filenames(
    filenames: list[str], pattern: str, add_stars: bool = False
) -> list[str]:
    ...
    if args.examples:
        ...
        if len(matching) > 0:
            ...
            for match in matching:
                ...
                newmatch += _match_filenames(data_files, data_pattern, add_stars=True)
            matching = newmatch
        ...
        if len(matching) > 0:
            for filename in matching:
                ...
                if os.path.isfile(filename):
                    ...
                    qr.printlog("Directory with the name", filename, "already exists!")
...
def do_command_file(args: Any) -> None:
    ...
    try:
        ...
        qr.printlog("Saved with Quantarhei version: " + check["qrversion"], loglevel=1)
...
def main() -> None:
    ...
    parser.add_argument(
        "-l", "--logtofile", action="store_true", help="copy logging into a file"
    )
    ...
    parser_run.add_argument(
        "-s", "--silent", action="store_true", help="logging level set to zero"
    )
    parser_run.add_argument(
        "-q", "--quiet", action="store_true", help="no output from qrhei script itself"
    )
    parser_run.add_argument(
        "-p", "--parallel", action="store_true", help="executes the code in parallel"
    )
    parser_run.add_argument(
        "-n", "--nprocesses", type=int, default=0, help="number of processes to start"
    )
    parser_run.add_argument(
        "-d",
        "--driver",
        type=str,
        default="mpirun",
        help="driver command, e.g. for MPI.",
    )
    parser_run.add_argument(
        "-f",
        "--hostfile",
        metavar="HOSTFILE",
        default="",
        help="list of available host for parallel calculation",
    )
    parser_run.add_argument(
        "-b",
        "--benchmark",
        type=int,
        default=0,
        help="run one of the predefined benchmark calculations",
    )
    parser_run.add_argument(
        "-i",
        "--inputfile",
    ...
    parser_fetch.add_argument(
        "glob", metavar="glob", type=str, help="file name", nargs="?"
    )
    parser_fetch.add_argument(
        "-e", "--examples", action="store_true", help="fetches a specified example file"
    )
    ...
    parser_list = subparsers.add_parser(
        "list", help="Lists examples, benchmarks, tutorials and templates"
    )
    ...
    parser_list.add_argument(
        "-e", "--examples", action="store_true", help="list all available example files"
    )
    ...
    parser_file = subparsers.add_parser(
        "file", help="Shows information about files created by Quantarhei"
    )

    parser_file.add_argument(
        "fname", metavar="fname", type=str, help="file to be checked", nargs=1
    )
    ...
    parser_script = subparsers.add_parser("script", help="Installs and manages scripts")
    ...
    parser_script.add_argument(
        "-i", "--install", metavar="SCRIPT", default="", help="installs a script file"
    )
    parser_script.add_argument(
        "-s",
        "--set-path",
        metavar="PATH",
        default="",
        help="sets path to Quantarhei scripts",
    )
    parser_script.add_argument(
        "-p",
        "--path",
    ...
    parser_script.add_argument(
        "-l", "--list", action="store_true", help="lists all installed scripts"
    )
    parser_script.add_argument(
        "-y",
        "--history",
        metavar="SCRIPT",
        default="",
        help="reports the history of an installed script",
    )
    parser_script.add_argument(
        "-t",
        "--status",
        metavar="SCRIPT",
        default="",
        help="reports the status of an installed script",
    )
    parser_script.add_argument(
        "-a",
        "--hash",
        metavar="FILE",
        default="",
        help="creates a hash unique to the content of a file",
    )
