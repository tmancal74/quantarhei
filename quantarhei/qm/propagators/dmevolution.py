...
class DensityMatrixEvolution(MatrixData, BasisManaged, Saveable):
    ...
    def set_initial_condition(self, rhoi: Any) -> None:
        ...
        self._data = numpy.zeros(
            (self.TimeAxis.length, self.dim, self.dim), dtype=numpy.complex128
        )
    ...
    def transform(self, SS: numpy.ndarray, inv: numpy.ndarray | None = None) -> None:
        ...
        for ii in range(self.TimeAxis.length):
            self._data[ii, :, :] = numpy.dot(S1, numpy.dot(self._data[ii, :, :], SS))
    ...
    def convert_from_RWA(self, ham: Any, sgn: int = 1) -> None:
        ...
        if (self.is_in_rwa and sgn == 1) or sgn == -1:
            ...
            for i, t in enumerate(self.TimeAxis.data):
                ...
                # revert RWA
                rhot = numpy.dot(Ut, numpy.dot(self.data[i, :, :], numpy.conj(Ut)))
                self.data[i, :, :] = rhot
    ...
    def plot(
        ...
        trace: bool = False,
        coherences: bool = True,
        cohselection: str = "All",
        how: str = "-",
        axis: Any = None,
        show: bool = True,
        legend: bool = False,
        ...
        if populations:
            for ii in range(start, N):
                ...
                while kk > 6:
                    kk = kk - 7
                plt.plot(self.TimeAxis.data, numpy.real(self.data[:, ii, ii]), howi[kk])
                leg.append(str(ii))
            ...
            for ii in range(N):
                for jj in range(ii + 1, N):
                    if ii >= start:
                        ...
                        plt.plot(
                            self.TimeAxis.data,
                            numpy.real(self.data[:, ii, jj]),
                            howi[kk],
                        )  # how)
                        plt.plot(
                            self.TimeAxis.data,
                            numpy.imag(self.data[:, ii, jj]),
                            "-" + howi[kk],
                        )
