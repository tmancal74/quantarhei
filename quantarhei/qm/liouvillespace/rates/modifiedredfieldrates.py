...
class ModifiedRedfieldRateMatrix:
    ...
    def _set_rates2(self, force_detbalance: bool = True) -> None:
        ...
        for ii in range(
            Na - 1
        ):  # assume single excitation per monomer - not correct one should replace it with: aggreg.Nb[1]
        ...
        for M in range(Na):
            ...
            for L in range(minL, Na):
                ...
                fw_full = (
                    2
                    * Nt
                    * numpy.fft.fftshift(numpy.fft.ifft(numpy.fft.fftshift(ft_full)))
                    * dt
                )
            ...
            for M in range(Na):
                for L in range(M + 1, Na):
                    ...
                    om00 = (
                        hD[M]
                        - hD[L]
                        + numpy.dot(SS[:, L] ** 4, lambdas)
                        - numpy.dot(SS[:, M] ** 4, lambdas)
                    )
    ...
    def _set_rates(self) -> None:
        ...
        for a in range(Na):
            for b in range(Na):
                for c in range(Na):
                    for d in range(Na):
                        lam4[a, b, c, d] = self.sbi.CC.get_reorganization_energy4(
                            a, b, c, d
                        )
...
def ssModifiedRedfieldRateMatrix(
    ...
    for a in range(Na):
        F_k_t[a, :] = numpy.exp(
            -1j * (E_0k[a] - lam4[a, a, a, a]) * tt[:] - g4[a, a, a, a, :]
        )
