        ...
        if isinstance(sbi.system, Aggregate):
            ...
            if agg.Nel == agg.Ntot or sbi.KK[0].shape[0] == agg.Ntot:
                ...
                if Nel1 == sbi.KK.shape[1]:
                    ...
                    newsbi = SystemBathInteraction(
                        sys_operators=ops, rates=sbi.rates, system=sbi.system
                    )
                    super().__init__(
                        ham,
                        newsbi,
...
class VibrationalDecayLindbladForm(LindbladForm):
    ...
    def __init__(
                ...
                for nk in range(nmax):
                    ...
                    for a, s1 in agg.all_states:
                        ...
                        for b, s2 in agg.all_states:
                            ...
                            if (el1 == el2) and self._same_regardless_of_one(
                                vb1, vb2, site
                            ):
                                if vb1[site] == pair[0] and vb2[site] == pair[1]:
        ...
        # with the operators constructed, we create Lindblad form
        newsbi = SystemBathInteraction(sys_operators=ops, rates=rts, system=sbi.system)
        super().__init__(
