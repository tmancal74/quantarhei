...
class NEFoersterRelaxationTensor(TDFoersterRelaxationTensor):
        ...
        with energy_units("int"):
            ...
            if self.nsc:
                if not self.as_kernel:
                    ...
                    self.data = self.td_reference_implementation(Na, Nt, HH, tt, gt, ll)
                    ...
                    self.kernel = self.td_reference_implementation(
                        Na, Nt, HH, tt, gt, ll
                    )
    ...
    def td_reference_implementation(
        ...
        if self.nsc:
            ...
            #
            if not self.as_kernel:
                return _nsc_reference_implementation(
                    Na, Nt, HH, tt, gt, ll, _nsc_fintegral
                )
            #
            # Here we prepare the kernel so that it can be returned
            # on demand for any value of propagation time t
            #
            return _nsc_kernel_implementation(Na, Nt, HH, tt, gt, ll, _nsc_kernel_at_t)
...
def _integrate_three_to_t(
    ...
    tt: numpy.ndarray,
    fce_t: numpy.ndarray,
    fce_tau: numpy.ndarray,
    fce_ttau: numpy.ndarray,
) -> Any:
