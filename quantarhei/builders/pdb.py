...
class PDBFile:
    ...
    def get_Molecules(self, model: object = None) -> list:
        ...
        #
        mollines = self._match_lines(by_recName="HETATM", by_resName=res_name)
        ...
        for rseq in self._uniqueIds:
            m = Molecule(name=str(rseq), elenergies=model.default_energies)
            r = model.position_of_center(data_type="PDB", data=self._unique_lines[rseq])
            m.position = r
            d = model.transition_dipole(data_type="PDB", data=self._unique_lines[rseq])
            m.set_dipole(0, 1, d)
            m.model = self
