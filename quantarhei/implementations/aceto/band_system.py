...
class band_system:
    ...
    def set_dipoles(self, Nbi: int, Nbf: int, dab: Any) -> None:
        ...
        if (Nbi == 0) and (Nbf == 1):
            ...
            for i in range(N1):
                for j in range(N2):
                    dd = numpy.sqrt(numpy.dot(self.nn12[:, i, j], self.nn12[:, i, j]))
                    self.dd12[i, j] = dd
                    if dd != 0.0:
    ...
    def init_dephasing_rates(self) -> None:
        ...
        self.Kd11 = numpy.zeros(
            (self.Ns[1], self.Ns[1]), dtype=numpy.float64, order="F"
        )
        self.Kd12 = numpy.zeros(
            (self.Ns[1], self.Ns[2]), dtype=numpy.float64, order="F"
        )
