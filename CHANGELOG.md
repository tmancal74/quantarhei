...
### For users
- Multi-level molecules
- Complete Pump-probe calculations
- Improved 2D calculations
...
### For users
- Quantarhei starts enforcing usage of `energy_units` context with some functions. When energy values are set, it will be compulsory to use the `energy_units` context manager. The introduction of this feature may take several Quantarhei versions.
- Bug fixes
...
- Improved logging capabilities; standard `print` function can be replaced by `printlog` function
- Bug fixes
...
- Better diagnostics of errors occurring while running a script by `qrhei` driver
...
### For users
- Bug fixes
- 2D spectra can be shifted in their axis by less than the frequency step (interpolated shift)
...
- Most classes can now be copied (`.copy()` for a shallow copy, `.deepcopy()` for a deep, recursive copy)
...
- Class migration: `MockTwoDSpectrum` → `MockTwoDResponseCalculator`
...
- All `Saveable` objects now have convenience methods `copy()` for shallow copying, `deepcopy()` for deep copying, and `scopy()` for a deep copy by saving to and loading the object from a temporary file
...
- Empty relaxation superoperator (as an empty Lindblad form) introduced (as a temporary fix to allow pure dephasing dynamics only)
...
- Some bug fixes including a frequency factor in absorption spectrum

## [0.0.41]

### For users
- Some bug fixes
- Better Liouville pathway manipulation features
...
- Improved `TwoDSpectrumContainer` (can hold a group of spectra identified by an arbitrary `ValueAxis`, integer index or list of strings). Copies the new storage improvement on `TwoDSpectrum`.
...
- New subpackage `quantarhei.testing` unites all custom functions that support testing. It includes `feature.py` module previously found in `quantarhei.dev` subpackage (now removed) and a `behave.py` module which supports tests with behave package
...
### For users
- Method `get_DensityMatrix()` of the `Aggregate` class improved. It accepts some new options which make specification of desired density matrix more flexible
...
### For developers
- The code is now hosted on travis-ci.com and the builds are tested after every commit
- The code is now hosted on codecov.com and its coverage by tests is measured
...
- `quantarhei.REAL` and `quantarhei.COMPLEX` types should now be used for numpy arrays throughout the package. These types can be controlled and with it the used numerical precision and memory needs
...
- Part of the test suite available for installed Quantarhei package
...
- pytest required to run new tests with matplotlib plots
